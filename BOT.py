...
# from config import token #токен от BotFather
...
#         r1 = bot.send_message(message.chat.id, result[3])
#         r2 = bot.send_message(message.chat.id, result[2])
#         r3 = bot.send_photo(message.chat.id, result[0])
#         r4 = bot.send_photo(message.chat.id, result[1])
#         check = bot.send_message(message.chat.id, '\n\n'
#                                      'Все в порядке? Тогда жми "Опубликовать"!\n'
#                                      'Что-то пошло не так? Тогда жми "Restart"')
#         bot.register_next_step_handler(check, vote)
#
#     elif message.text == '✅ Опубликовать':
#         CHANNEL_ID = '-1001814912527'
#         result = bd.bd_check_add(hash_vote)
#         r1 = bot.send_document(CHANNEL_ID, result[3], result[2], result[0], result[1])
#
#         public = bot.send_message(message.chat.id, 'Теперь ты можешь найти свой Vote здесь:\n'
#                          '@VoteForPic\n\n'
#                                                    'Нажми любую кнопку, чтобы начать заново: ')
#         bot.register_next_step_handler(public, button)
#
#     elif message.text == '🔁 Restart':
#         rstrt = bot.send_message(message.chat.id, 'Понял, давай всё по новой 😌!\n'
#                                                   'Жми любую кнопку:')
#         bot.register_next_step_handler(rstrt, button)
#
#
